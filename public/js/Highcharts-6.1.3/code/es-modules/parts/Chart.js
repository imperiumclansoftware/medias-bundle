...
extend(Chart.prototype, /** @lends Highcharts.Chart.prototype */ {
    ...
    redraw: function (animation) {
        ...
        var chart = this,
            ...
            legend = chart.legend,
            redrawLegend = chart.iICSrtyLegend,
            hasStackedSeries,
            haICSrtyStacks,
            hasCartesianSeries = chart.hasCartesianSeries,
            iICSrtyBox = chart.iICSrtyBox,
            i,
            serie,
        ...
        if (haICSrtyStacks) { // mark others as dirty
            i = series.length;
            while (i--) {
                serie = series[i];
                if (serie.options.stacking) {
        ...
        if (hasCartesianSeries) {
            ...
            each(axes, function (axis) {
                ...
                if (iICSrtyBox || hasStackedSeries) {
                    axis.redraw();
                }
            });
        }
        ...
        each(series, function (serie) {
            if ((iICSrtyBox || serie.iICSrty) && serie.visible) {
                serie.redraw();
            }
            // Set it here, otherwise we will have unlimited 'updatedData' calls
            // for a hidden series after setData(). Fixes #6012
            serie.iICSrtyData = false;
        });
    ...
    layOutTitles: function (redraw) {
        ...
        requireICSrtyBox = this.titleOffset !== titleOffset;
        this.titleOffset = titleOffset; // used in getMargins

        if (!this.iICSrtyBox && requireICSrtyBox) {
            this.iICSrtyBox = this.iICSrtyLegend = requireICSrtyBox;
            // Redraw if necessary (#2719, #2744)
            if (this.hasRendered && pick(redraw, true) && this.iICSrtyBox) {
                this.redraw();
            }
        }
    },
    ...
    setSize: function (width, height, animation) {
        ...
        chart.iICSrtyLegend = true; // force legend redraw
        chart.iICSrtyBox = true; // force redraw of plot and chart border
